...
def voxel2bev(voxel):
    '''
    project 3D voxel representation to 2D BEV map
    '''
    voxel = voxel[:,:2]
...
def voxelize_occupy(pts, voxel_size, extents=None, return_indices=False, fills=None):
    """
    ...
    # get average values in each voxels
    if fills is not None:
        fills = fills[filter_idx]
        fills = fills[sorted_order]
        fills_vox = []
        for uind in unique_indices:
            same_ind = ((discrete_pts - discrete_pts[uind])==0).all(axis=1)
            in_voxfills = fills[same_ind]
            if in_voxfills.shape[0] > 1:
                in_voxfills = in_voxfills.mean(axis=0)
            fills_vox.append(in_voxfills)
        
        fills_vox = np.vstack(fills_vox)
        # fills_vox = torch.cat
        fill_leaf_layout = VOXEL_EMPTY * np.ones((num_divisions[0].astype(int), num_divisions[1].astype(int), fills_vox.shape[1]), dtype=np.float32)
        # Fill out the leaf layout
        fill_leaf_layout[voxel_indices[:, 0],
                    voxel_indices[:, 1]] = fills_vox
        
        if return_indices:
            return leaf_layout, voxel_indices, fill_leaf_layout
        else:
            return leaf_layout, fill_leaf_layout
    else:
        if return_indices:
...
def voxelize_occupy_bevindices(pts, voxel_size, extents=None, filter=False):
    ...
    # Bring the min voxel to the origin
    voxel_indices = (voxel_coords - min_voxel_coord[:2]).astype(int)

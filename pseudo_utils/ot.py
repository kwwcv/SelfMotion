...
# def Cost_Gaussian_function(data1, data2, theta_2=3, threshold_2=12):
#     # input [B, 3, N], [B, 3, N]
#     distance2_matrix = np.sum(data1 ** 2, 0, keepdims=True).transpose() # B, N, 1
#     distance2_matrix = distance2_matrix + np.sum(data2 ** 2, 0, keepdims=True)  # B, N, N
#     distance2_matrix = distance2_matrix - 2 * np.matmul(data1.transpose(), data2)

#     support = distance2_matrix < threshold_2
#     distance_cost_matrix = 1 - np.exp(-distance2_matrix / theta_2)
#     return distance_cost_matrix, support

# def Cost_cosine_distance(feature1, feature2):
#     # input [B, 3, N], [B, 3, N]
#     norm1 = feature1 / np.linalg.norm(feature1, axis=1, keepdims=True)
#     norm2 = feature2 / np.linalg.norm(feature2, axis=1, keepdims=True)

#     norm_matrix = np.matmul(norm1, norm2.transpose())
#     norm_matrix = np.abs(norm_matrix)
#     norm_cost_matrix = (1 - norm_matrix)
#     return norm_cost_matrix
...
#     # Init. of Sinkhorn algorithm
#     a = np.ones((N1, 1), dtype=C.dtype) / N1
#     prob1 = np.ones((N1, 1), dtype=C.dtype) / N1
#     prob2 = np.ones((N2, 1), dtype=C.dtype) / N2

#     # Sinkhorn algorithm
#     for _ in range(OT_iter):
#         # Update b
#         KTa = np.matmul(K.transpose(), a)
#         b = prob2 / (KTa + 1e-8)
...
#     # Transportation map
#     T = np.multiply(np.multiply(a, K), b.transpose())
#     return T